...
async def unicorn_exception_handler(request: Request, exc: GenericException):
    return JSONResponse(
...
async def already_performed_exception_handler(request: Request, exc: AlreadyPerformedException):
    return JSONResponse(
        status_code=exc.code,
        content={ "message": exc.msg },
    )
