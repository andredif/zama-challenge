...
case "${1:-}" in
    ...
    -migrations)
        echo "$0: ${1#-}"
        set -- python -m alembic upgrade head
        ;;
    ...
    -startserver-debug)
        ...
        python -m debugpy --listen 0.0.0.0:5678 -m zama-challenge.main --port $PORT --host $HOST --debug
        # uvicorn --reload --port $PORT --host $HOST --log-level debug \
        #         app:app
                --ssl-certfile ${RESOURCES_PATH}/src/ssl/server.crt \
                --ssl-keyfile ${RESOURCES_PATH}/src/ssl/server.key \
                # --ssl-ca-certs ${RESOURCES_PATH}/etc/ssl/ca_path_info \
